...
let KY_CONGRESS_GEOJSON = null;
let KY_HOUSE_GEOJSON = null;
let KY_SENATE_GEOJSON = null;
...
function addLayers() {
  ...
  map.addSource('kentuckyFema', { type: 'geojson', data: 'data/KY_FEMA_County.json' });
...
function addCongressionalLayers() {
  fetch('data/KY_Congress.json')
    .then(r => r.json())
    .then(data => {
      KY_CONGRESS_GEOJSON = data;
      map.addSource('kyCongress', { type: 'geojson', data });
...
function addHouseLayers() {
  fetch('data/KY_House.json')
    .then(r => r.json())
    .then(data => {
      KY_HOUSE_GEOJSON = data;
      map.addSource('kyHouse', { type: 'geojson', data });
...
function addSenateLayers() {
  fetch('data/KY_Senate.json')
    .then(r => r.json())
    .then(data => {
      KY_SENATE_GEOJSON = data;
      map.addSource('kySenate', { type: 'geojson', data });
      ...
      map.addLayer({
        ...
        layout: {
          'visibility': 'none',
          'text-field': ['get', 'OBJECTID'], // <-- senate district field
          'text-font': ['Open Sans Bold', 'Arial Unicode MS Bold'],
          'text-size': 14
        },
        paint: {
...
function getDistrictFeaturesFromMemory(lngLat) {
  ...
  addHits(KY_CONGRESS_GEOJSON, 'congressionalDistricts');
  addHits(KY_HOUSE_GEOJSON, 'houseDistricts');
  addHits(KY_SENATE_GEOJSON, 'senateDistricts');
